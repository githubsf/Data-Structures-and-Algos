 ...
 * Return if input is a single data element.
 * If multiple data items are in input, split in half and sort each half.
 * Merge the sorted halves back together.
This algorithm is known as a divide-and-conquer technique. It divides the set in half and then attacks each half independantly. Within each half it repeats itself dividing the set again in half and attacking each half.. Thus, if the list to be sorted consisted of eight items, merge sort would break the set up into sets of four, then sets of two, then sets of one. Then it would merge each set back together, placing items in appropriate order according to the set they are currently in and the set they are being merged with. The structure looks as follows:
All Items
     1/2                                    1/2
   1/4 1/4                              1/4     1/4
...
 
	...
	private static void recSortMerge(int[] workspace, int low, int high)
	{
		...
		/// base case
		if( low == high) return;   // if there is only one element in range, no sorting needed
		else
		{
	 ...
	 private static void recMerge(int[] workspace, int begLow, int begHigh, int endHigh)
	{
		...
		int j=0; // the index of the temporary array data structure that
		         // will be in memory, this is the overhead of Merge Sort
